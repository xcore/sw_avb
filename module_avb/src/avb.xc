...
static unsafe void set_sink_state0(int sink_num, enum avb_sink_state_t state, chanend c_mac_tx, chanend ?c_media_clock_ctl) {
...
static unsafe void set_source_state0(int source_num, enum avb_source_state_t state, chanend c_mac_tx, chanend ?c_media_clock_ctl) {
...
unsafe void avb_process_control_packet(unsigned int buf0[], int nbytes, chanend c_tx, chanend ?c_media_clock_ctl, unsigned int port_num)
{
  if (nbytes == STATUS_PACKET_LEN) {
    ...
    unsafe {
      unsigned char *unsafe buf = (unsigned char *unsafe) buf0;
