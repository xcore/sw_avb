  ...
  while (1)
  {
    select
    {
      ...
      case !buttons_active => button_tmr when timerafter(buttons_timeout) :> void:
        ...
        p_buttons :> button_val;
        break;
      case c_gpio_ctl :> unsigned int mode:
        if(mode)
        {
          p_chan_leds <: 0;
        }
  ...
  while (1)
  {
    ...
    select
    {
      ...
      case avb_get_control_packet(c_rx, buf, nbytes):
      {
        ...
        newIdentify = get_aem_identify_control_value();
        
        if(newIdentify != identify_state)
        {
          unsigned int val = newIdentify;
          identify_state = newIdentify;
          c_gpio_ctl <: val;
        }
        break;
      }
      ...
      case c_gpio_ctl :> int cmd:
      {
        switch (cmd)
        {
          ...
          {
            toggle_remote = !toggle_remote;
            avb_1722_1_aecp_aem_identify_set_button_pressed(toggle_remote);
            break;
          }
          break;
        }
