...
void demo(chanend talker_ctl[], chanend media_ctl[], chanend media_clock_ctl,
		chanend c_ptp, chanend tcp_svr, chanend c_rx, chanend c_tx);
...
on stdcore[0]: out port p_led = PORT_LED_0_1;
...
int main(void)
{
	...
	par
	{
		...
		// TCP/IP stack
		on stdcore[1]: uip_server(rx_link[1], tx_link[2], xtcp, 1, null, connect_status);
		...
		on stdcore[0]: avb_1722_talker(ptp_link[0], tx_link[1], talker_ctl[0], AVB_NUM_SOURCES);

		// Xlog server
		on stdcore[0]: xlog_server_uart(p_uart_tx);

		// Application threads
		on stdcore[0]: demo(talker_ctl, media_ctl, media_clock_ctl, ptp_link[2], xtcp[0], rx_link[2], tx_link[3]);
...
void demo(chanend talker_ctl[], chanend media_ctl[], chanend media_clock_ctl, chanend c_ptp, chanend tcp_svr, chanend c_rx, chanend c_tx)
{
	timer tmr;
	int avb_status = 0;
	int map[8];
	...
	unsigned timeout;
#if AVB_DFU_UPGRADE_IMAGE_TEST
	unsigned led_on = 1, counter = 0;
#endif

	// First initialize avb higher level protocols
	avb_init(media_ctl, null, talker_ctl, media_clock_ctl, c_rx, c_tx, c_ptp);
	...
	// Firmware upgrade over TFTP
#ifdef AVB_ENABLE_TFTP_DFU
	tftp_init(tcp_svr);
#endif
...
#ifdef AVB_ENABLE_TFTP_DFU
				tftp_handle_event(tcp_svr, conn);
#endif
